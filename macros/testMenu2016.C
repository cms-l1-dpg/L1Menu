...
int main ( int argc, char *argv[] )
{
  ...
  desc.add_options()
    ...
    ("doBXReweight",   po::bool_switch()->default_value(false),                "8b4e bx reweighting to 48b")
    ...
    ("BXReweight_first",       po::value<int>(),                               "index of first BX in 8b4e")
    ("SumJetEta",      po::value<float>(),                                     "Eta threshold of Jet for HT")
    ("SetMuonER",      po::value<float>(),                                     "Set the ER in eta for Muon")
    ("SetNoPrescale",  po::bool_switch()->default_value(false),                "Set all prescales of prescalsed Seeds to 1")
